...
gezegenler_dir = os.path.join(project_dir, 'gezegenler')
evler_dir = os.path.join(project_dir, 'evler')

os.makedirs(gezegenler_dir, exist_ok=True)
os.makedirs(evler_dir, exist_ok=True)

# 1. Gezegen Sayfalarını Üret
planets_path = os.path.join(project_dir, 'data', 'planets.json')
if os.path.exists(planetsPath := planets_path):
    with open(planetsPath, 'r', encoding='utf-8') as f:
        planets = json.load(f)
    for p in planets:
        house_cards = ""
        for hp in p['housePositions']:
            house_cards += f"""
        ...
        accent_color = p.get('accentColor', '#d4af37')
        accent_rgb = "212,175,55"
        if accent_color == '#ef476f':
            accent_rgb = "239,71,111"
        elif accent_color == '#8296c9':
            accent_rgb = "130,150,201"
...
<body>
  ...
  <footer class="detail-footer">
    <p>© 2026 Zodyak Atlası — Gökyüzünün Kadim Bilgeliği</p>
        ...
        with open(os.path.join(gezegenler_dir, f"{p['slug']}.html"), 'w', encoding='utf-8') as out:
            out.write(html_content)
        print(f"Generated planet page: {p['slug']}.html")

# 2. Ev Sayfalarını Üret
houses_path = os.path.join(project_dir, 'data', 'houses.json')
if os.path.exists(housesPath := houses_path):
    with open(housesPath, 'r', encoding='utf-8') as f:
        houses = json.load(f)
    for h in houses:
        html_content = f"""<!DOCTYPE html>
<html lang="tr">
...
<body>
  ...
  <footer class="detail-footer">
    <p>© 2026 Zodyak Atlası — Gökyüzünün Kadim Bilgeliği</p>
        ...
        with open(os.path.join(evler_dir, f"{h['slug']}.html"), 'w', encoding='utf-8') as out:
            out.write(html_content)
        print(f"Generated house page: {h['slug']}.html")
